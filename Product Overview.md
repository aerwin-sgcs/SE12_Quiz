...
| `match`  | `pairs[]` (term/def tuples)  | All pairs correct                                    |
...
**Teacher flow:**
1. Click "Teacher view" on the home screen
2. Enter PIN (default `1234`, stored in `localStorage` on that one device)
...
- No localStorage for student data — only for teacher PIN
